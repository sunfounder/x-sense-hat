...
class RGB_Matrix():
    ...
    def __init__(self, addr):
        ...
        self.transpose_image = Image.new('RGB', (self.width, self.height))
        self.draw = ImageDraw.Draw(self.new_image)
        self.font = ImageFont.truetype('/home/pi/sensor_hat/sensor_hat/Minecraftia-Regular.ttf', 8)
    ...
    def draw_line(self,coor_1,coor_2,fill,width=0, joint=None):
        coor = [coor_1[0],coor_1[1],coor_2[0],coor_2[1]]
    ...
    def draw_ellipse(self,coor,radius,fill,outline=None, width=0):
        # if outline == True:
        #     self.draw.ellipse([coor[0]-radius,coor[1]-radius,coor[0]+radius,coor[1]+radius],fill=None,outline=fill, width=0)
        # else:
        #     self.draw.ellipse([coor[0]-radius,coor[1]-radius,coor[0]+radius,coor[1]+radius],fill=fill,outline=None, width=0)
        # coor = [coor_1[0],coor_1[1],coor_2[0],coor_2[1]]    
    ...
    def clear_screen(self):
        self.draw.rectangle((0,0,self.width,self.height), outline=None, fill=0)
        # self.display()
        # img = list(self.new_image.getdata())
        # self.image(img)

    def display(self):
        self.transpose_image = self.new_image.transpose(Image.FLIP_TOP_BOTTOM)
        img = list(self.transpose_image.getdata())
        self.image(img)    
    ...
    def show_icon(self, icon, color):
        ...
        _bytes = self.string_bits_to_bytes(self.icons(icon))
        self.display_char(_bytes, self.rgb(color[0],color[1],color[2]))
